...
> **让原本不支持 DLNA 的小爱音箱，也能接收手机音乐 App 的投屏。**

它在手机上跑一层 **DLNA / UPnP 中转**：

- **对音乐 App**，它伪装成一台标准 DLNA 音箱 —— SSDP 应答 + UPnP 设备描述 + SOAP 控制 + GENA 事件订阅齐全，
  网易云音乐、QQ 音乐（QPlay）都能正常发现并投屏；
- **对音箱**，它把收到的投屏指令翻译成**小米云 API** 调用（`player_play_music` / `player_play_url`）下发下去，
  音箱只知道自己收到了一条播放指令。DLNA 投屏只传**音频地址**、不传音频本身，所以全程**不需要解码、不需要转码**；
- **中间的音频流**由手机上的本地 HTTP 代理接住转发，支持 Range / seek，可选择先落盘缓冲以抗 CDN 抖动与重定向。

**零额外硬件**：不需要 NAS、不需要服务器、不需要软路由或树莓派，也不需要 root —— **装个 APK 就行**。
...
| 主页 | 二级页（设置） |
|:------|:------|
| ![主页](docs/screenshots/01-home.png) | ![设置](docs/screenshots/02-settings.png) |
